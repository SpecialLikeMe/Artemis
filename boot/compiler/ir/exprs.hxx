...
inline LLVMValueRef visit_lvalue(expr_node* e, ir_context* ctx) {
    switch (e->kind) {
        ...
        case expr_kind::unary: {
            ...
            // Emit null check for write-through-pointer if flagged unsafe
            if (ctx->unsafe_ops.count(e)) {
                LLVMValueRef cond = LLVMBuildIsNotNull(ctx->llvm_builder, ptr, "nn_wchk");
...
inline LLVMValueRef visit_subscript_expr(expr_node* e, ir_context* ctx) {
    ...
    if (ctx->unsafe_ops.count(e)) {
        ...
        if (arr_t && LLVMGetTypeKind(arr_t) == LLVMArrayTypeKind) {
            ...
            // Dynamic array (raw pointer): upper bound cannot be checked without size info.
            // Negative signed indices are caught by signed >= 0 check; upper bound
            // requires fat-pointer/slice types that carry length — architectural gap.
            LLVMValueRef zero = LLVMConstInt(LLVMTypeOf(idx), 0, 0);
            cond_ok = LLVMBuildICmp(ctx->llvm_builder, LLVMIntSGE, idx, zero, "bchk_lo");
        }

...
inline void visit_union_decl(union_decl* d, ir_context* ctx) {
    ...
    for (auto* f : d->fields) {
        ...
        // Use type-kind-aware sizing to avoid calling LLVMGetIntTypeWidth on non-integer types
        // (it returns 0 for float/struct/pointer types, corrupting the widest-field selection).
        unsigned fw = 0;
        switch (LLVMGetTypeKind(ft)) {
            case LLVMIntegerTypeKind:  fw = LLVMGetIntTypeWidth(ft); break;
            case LLVMFloatTypeKind:    fw = 32; break;
            case LLVMDoubleTypeKind:   fw = 64; break;
            case LLVMHalfTypeKind:     fw = 16; break;
            case LLVMFP128TypeKind:    fw = 128; break;
            case LLVMPointerTypeKind:  fw = 64; break; // target pointer size
            default:                   fw = 32; break; // conservative fallback for struct/array/vector
        }
...
inline void visit_top_level_decl(ast_node* node, ir_context* ctx) {
    ...
    if (dynamic_cast<memstr_decl*>(node))               { return; } // memstr vtable/type emission: deferred pending final memstr IR lowering design

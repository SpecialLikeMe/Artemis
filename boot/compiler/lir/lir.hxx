...
inline void mem2reg(mir_func& func) {
    ...
    for (auto& lbl : order) {
        ...
        // Move the block's instructions to a working copy before iterating.
        // read_variable_recursive may insert phi instructions at the front of
        // bb->instrs; if we iterated bb->instrs directly, that insert could
        // reallocate the vector and invalidate the range-for iterator.
        // By working from 'work', insertions go into the now-empty bb->instrs
        // without touching our iterator.
        std::vector<mir_instr> work = std::move(bb->instrs);
        bb->instrs.clear();

        std::vector<mir_instr> new_instrs;
        // Persistent rename map: when a load_local is eliminated, map its dest tmp → the SSA value.
        // Applied to every subsequent instruction so forward uses are correctly renamed.
        std::unordered_map<std::string, mir_val> rename_map;

        auto apply_renames = [&](mir_val& mv) {
            if (mv.kind == mvk::tmp) {
                auto it = rename_map.find(mv.name);
                if (it != rename_map.end()) mv = it->second;
            }
        };
        ...
        for (auto& i : work) {
            ...
            if (i.kind == mik::store_local && !i.ops.empty() &&
                ...
                // Apply any pending renames to the stored value first
                mir_val stored_val = i.ops[0];
                apply_renames(stored_val);
                ctx.write_variable(i.ops[1].name, lbl, stored_val);
                continue;
            }
            if (i.kind == mik::load_local && !i.ops.empty() &&
                promotable.count(i.ops[0].name)) {
                // load_local slot → read_variable(slot, blk)
                // This may insert phi nodes at the front of bb->instrs (safe
                // since we are iterating 'work', not bb->instrs).
                mir_val v = ctx.read_variable(i.ops[0].name, lbl);
                // Record that dest tmp now equals v — future instructions will be renamed
                ctx.write_variable(i.dest, lbl, v);
                rename_map[i.dest] = v;
                continue;
            }
            // Apply renames to this instruction's operands before pushing
            for (auto& op : i.ops) apply_renames(op);
            for (auto& pa : i.phi_args) apply_renames(pa.val);
            new_instrs.push_back(std::move(i));
        }
        // Apply renames to terminator operands
        apply_renames(bb->term.cond);
        apply_renames(bb->term.ret_val);
        // bb->instrs now contains any phi nodes inserted during processing
        // (in front, by read_variable_recursive).  Append the non-eliminated
        // regular instructions after them.
        bb->instrs.insert(bb->instrs.end(),

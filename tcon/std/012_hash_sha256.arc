...
i32 main() {
    ...
    std.hash.sha256_digest d = std.hash.sha256_hash_str("");
    ...
    std.hash.sha256_digest d2 = std.hash.sha256_hash_str("");
    for (i32 i = 0; i < 32; i = i + 1) {
        if (d.bytes[i] != d2.bytes[i]) { printf("FAIL sha256 deterministic\n"); return 6; }
    }

    // Different inputs differ
    std.hash.sha256_digest d3 = std.hash.sha256_hash_str("a");

...
pub fn main() i32 {
    let mut a: std.alloc.arena((u64)8192);
    if (a.base == (void*)0) { return 1; }
    if (a.remaining() != 8192) { return 2; }

    // Alloc and write
    let mut p: *i32= (i32*)a.alloc_bytes(sizeof(i32) * 3);
    if (p == (i32*)0) { return 3; }
    p[0] = 1; p[1] = 2; p[2] = 3;
    ...
    // Save checkpoint
    a.save();
    let mut cp: u64= a.checkpoint;

    // Alloc more
    let mut q: *i32= (i32*)a.alloc_bytes(sizeof(i32) * 2);
    if (q == (i32*)0) { return 5; }
    ...
    let mut zp: *u8= (u8*)a.alloc_zeroed((u64)16);
    if (zp == (u8*)0) { return 9; }

...
i32 main() {
    ...
    u64 pos = 0;
    u32 cp = std.encode.utf8_decode_one(ascii, (u64)3, &pos);
    if (cp != 65u) { printf("FAIL decode ASCII\n"); return 1; }
    if (pos != 1) { printf("FAIL decode pos\n"); return 2; }

    // Encode ASCII back
    u8 enc[4];
    i32 n = std.encode.utf8_encode_one(65u, enc);
    ...
    i32 cnt = std.encode.utf8_count(str, (u64)3);
    if (cnt != 3) { printf("FAIL count\n"); return 6; }

    // Two-byte encode/decode round-trip (U+00E9 = é)
    i32 enc_n = std.encode.utf8_encode_one(0xE9u, enc);
    if (enc_n != 2) { printf("FAIL encode 2-byte len\n"); return 7; }
    pos = 0;
    u32 cp2 = std.encode.utf8_decode_one(enc, (u64)2, &pos);

...
struct mir_val {
    ...
    static mir_val null_()   { return {mvk::null_val}; }
...
static mir_val lower_expr(expr_node* e, mir_ctx& ctx) {
    ...
    case expr_kind::null_coal:
        // lhs ?? rhs — model lhs as the result (conservative: always emits UNKNOWN for null check)
        return lower_expr(e->lhs, ctx);
...
static void lower_stmt(ast_node* n, mir_ctx& ctx) {
    ...
    // All other statements (defer, switch, asm, etc.) — opaque
...
static void lower_func(func_decl* fd, mir_module& mod) {
    ...
    if (!ctx.terminated && !func.blocks.empty())
        func.blocks.back().term = {mtk::ret_void_};

...
static void sta_file(const std::string& path, const std::string& compiler,
    ...
    // --analyze-only: runs preprocess→lex→parse→analyze then exits 0 (clean) or 1 (errors).
    // No object file is written, so this works on all platforms without temp-file issues.
    std::string check_cmd = "\"" + compiler + "\" \"" + path + "\" --analyze-only" + flags + " 2>&1";

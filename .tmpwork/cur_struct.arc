...
//
//   mmap    : (meta: *void, align: usize, size: usize)              !*void
//   rsmap   : (meta: *void, data: *void, size: iofs)                bool
//   rmap    : (meta: *void, align: usize, data: *void, size: iofs)  !*void
//   free    : (meta: *void, data: *void)                            !void
//   destroy : (meta: *void)                                         !void
//
// `meta` comes first in every slot so a slot's type is exactly the type of the
// allocator method that fills it (`self` is the metadata pointer). PAST_TODO lists
// alignment first; that ordering would force a thunk per slot for no benefit.
//
...
struct __vtable__ {
    let mmap:    [](*void, usize, usize)!*void;
    let rsmap:   [](*void, *void, iofs)bool;
    let rmap:    [](*void, usize, *void, iofs)!*void;
...
istruc memstr {
    ...
    fn mmap(self: *memstr, size: usize) !*void {
        ...
        return try self.vtable.mmap(self.ptr, self.default_align(), size);
    }

    // Allocate `size` bytes aligned to `align`.
    fn mmap_aligned(self: *memstr, align: usize, size: usize) !*void {
        if (self.vtable.mmap == (void*)0) { return error.Unsupported; }
        return try self.vtable.mmap(self.ptr, align, size);
    }
    ...
    fn rmap(self: *memstr, data: *void, size: iofs) !*void {
        ...
        return try self.vtable.rmap(self.ptr, self.default_align(), data, size);
    }

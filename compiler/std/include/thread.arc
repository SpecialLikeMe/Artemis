...
istruc mutex {
    ...
    void lock(&self)     { pthread_mutex_lock((void*)self.handle); }
    void unlock(&self)   { pthread_mutex_unlock((void*)self.handle); }
    bool try_lock(&self) { return pthread_mutex_trylock((void*)self.handle) == 0; }

    void deinit(&self) { pthread_mutex_destroy((void*)self.handle); }
}

// --- scoped lock guard ---
istruc lock_guard {
    mutex* mtx;
    void __construct__(&self, mutex* m) { self.mtx = m; (*m).lock(); }
    void release(&self) { (*self.mtx).unlock(); }
}
...
istruc cond_var {
    ...
    void wait(&self, mutex* mtx) {
        pthread_cond_wait((void*)self.handle, (void*)(*mtx).handle);
    }

    void signal(&self)    { pthread_cond_signal((void*)self.handle); }
    void broadcast(&self) { pthread_cond_broadcast((void*)self.handle); }

    void deinit(&self) { pthread_cond_destroy((void*)self.handle); }
}
...
istruc semaphore {
    ...
    void __construct__(&self, u32 initial_val) {
        sem_init((void*)self.handle, 0, initial_val);
    }

    void post(&self)     { sem_post((void*)self.handle); }
    void wait(&self)     { sem_wait((void*)self.handle); }
    bool try_wait(&self) { return sem_trywait((void*)self.handle) == 0; }

    void deinit(&self) { sem_destroy((void*)self.handle); }
}
...
istruc thread_t {
    ...
    void spawn(&self, void*(void*)* fn, void* arg) {
        pthread_create(&self.handle, (void*)0, fn, arg);
        self.joined = false;
    }
    ...
    bool is_done(&self) { return self.joined; }
}
...
istruc once_flag {
    ...
    void __construct__(&self) { self.done = 0; }

    void call_once(&self, void()* fn) {
...
istruc thread_pool {
    ...
    void __construct__(&self, i32 n, void*(void*)* fn, void* arg, &memstr a) {
        self.count   = n;
    ...
    void deinit(&self, &memstr a) { a.deinit(self.threads); }
}

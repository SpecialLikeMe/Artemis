    ...
    for (const auto& i : blk.instrs) {
        ...
        if (i.kind == mik::arr_ptr_) {
            ...
            if (i.arr_size > 0) {
                ...
                // Dynamic array: cannot prove bounds without size info
                // Check if idx is provably non-negative and the array is non-null
                if (idx.lo >= 0) v = verdict::unknown; // non-negative but size unknown
                else v = verdict::unknown;
            }
            sites.push_back({i.kind, v, i.line, i.src});
            if (v == verdict::bad) { state.reachable = false; return state; }
        ...
        if (i.kind == mik::field_) {
            ...
            abs_ptr ap = state.eval_ptr(i.ops[0]);
            // Only a safety concern if base is a pointer; for value types it's always fine
            if (ap.definitely_null()) {
                sites.push_back({mik::deref_, verdict::bad, i.line, i.src});
                state.reachable = false; return state;
            }
            // Result is a field pointer — likely non-null if base is non-null
            if (ap.definitely_nonnull()) state.set_ptr(i.dest, abs_ptr::non_null());
            else state.set_ptr(i.dest, abs_ptr::maybe());
            continue;
        }
        ...
        if (i.kind == mik::store_local || i.kind == mik::load_local) {
            ...
            if (!i.dest.empty()) state.set_int(i.dest, abs_int::top());
        }
    }
    return state;
}
...
static func_analysis analyze_func(const mir_func& func) {
    ...
    for (int iter = 0; iter < MAX_ITERS && !worklist.empty(); ++iter) {
        ...
        auto propagate = [&](const std::string& succ_lbl, abs_state succ_state) {
            ...
            // Widening after a few iterations to ensure termination
            if (iter >= 5) {

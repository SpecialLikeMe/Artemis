...
void visit_namespace_decl(parser.namespace_decl* nd, ir_context* ctx) {
    ...
    if (nd.is_istruc) { ctx.current_class_name = nd.name; }
    ...
    // Overloading is not supported — duplicate function names emit an error.
    // Forward declarations (no body) always get the base name and don't count as duplicates.
    i8** ol_qnames = (i8**)arc_malloc(sizeof(i8*) * (u64)(nd.decls_len + 1));
    i8*  seen_names[64];
    i32  seen_len = 0;
    i32 pre = 0;
    while (pre < nd.decls_len) {
        parser.ast_node* pd = nd.decls[pre];
        if (pd != (parser.ast_node*)0 && pd.kind == nd_func_decl) {
            parser.func_decl* pfd = (parser.func_decl*)pd;
            i8 base[512];
    ...
    while (pre0 < nd.decls_len) {
        parser.ast_node* pd0 = nd.decls[pre0];
        if (pd0 != (parser.ast_node*)0 && pd0.kind == nd_var_decl) {
            parser.var_decl* pvd = (parser.var_decl*)pd0;
    ...
    while (i < nd.decls_len) {
        ...
        if (decl != (parser.ast_node*)0) {
            ...
            if (k == nd_namespace_decl) {
                parser.namespace_decl* inner_nd = (parser.namespace_decl*)decl;
                if (inner_nd.type_params_len == 0 && inner_nd.decls_len > 0) {
                    parser.ast_node* first_child = inner_nd.decls[0];
                    if (first_child != (parser.ast_node*)0 && first_child.kind == nd_struct_decl) {
                        parser.struct_decl* inner_sd = (parser.struct_decl*)first_child;
                        visit_struct_decl(inner_sd, ctx);
                        i8* inner_st = st_map_get(&ctx.struct_types, inner_sd.name);
                        if (inner_st != (i8*)0) {
                            i8 inner_qn[512];
            ...
            if (k == nd_func_decl) {
                ...
                if (qname_dup != (i8*)0) {
                    ...
                    if (fd.is_extern_c) {
                        i8* fn_p = sv_map_get(&ctx.global_funcs, orig_name);
                        i8* ft_p = st_map_get(&ctx.global_func_types, orig_name);
                    ...
                    if (saved_ns != (i8*)0) {
                        ...
                        i8* fn_p = sv_map_get(&ctx.global_funcs, fd.is_extern_c ? orig_name : qname_dup);
                        i8* ft_p = st_map_get(&ctx.global_func_types, fd.is_extern_c ? orig_name : qname_dup);
    ...
    while (j < nd.decls_len) {
        ...
        if (decl != (parser.ast_node*)0) {
            ...
            if (k == nd_var_decl)  {
                ...
                if (vd.is_static) {
                    i32 cls_len = (i32)strlen(nd.name);
                    i32 skip = cls_len + 9; // skip "CLASSNAME__static_"
                    i32 vn_len = (i32)strlen(vd.name);
                    if (vn_len > skip) {
                        i8 ns_alias[512];
            ...
            if (k == nd_func_decl) {
                ...
                if (qname_dup != (i8*)0) {
                    ...
                    if (saved_ns != (i8*)0) {
                        ...
                        i8* fn_ref = sv_map_get(&ctx.global_funcs, qname_dup);
                        i8* ft_ref = st_map_get(&ctx.global_func_types, qname_dup);
...
void visit_program(parser.program_node* prog, ir_context* ctx) {
    ...
    while (pre_tl < prog.decls_len) {
        parser.ast_node* ptl = prog.decls[pre_tl];
        if (ptl != (parser.ast_node*)0 && ptl.kind == nd_var_decl) {
            parser.var_decl* pvd = (parser.var_decl*)ptl;

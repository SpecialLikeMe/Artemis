...
struct struct_meta {
    ...
    bool       is_istruc;   // true if declared as 'istruc' (class-like, has methods)
}
...
struct ir_scope_frame {
    ...
    sb_map alloca_volatile; // name -> bool (volatile qualifier)
...
struct ir_context {
    ...
    bool current_ns_is_istruc;  // true when inside an istruc namespace block
    ...
    i8* memstr_fat_type;       // %__memstr_fat__ = { ptr data, ptr vtable }
    i8* memstr_vtable_type;    // { ptr mmap_fn, ptr rmap_fn, ptr deinit_fn }
    sv_map memstr_vtables;     // class_name -> vtable global (LLVMValueRef as i8*)
    ...
    sv_map generic_classes;     // qualified name -> namespace_decl* (i8*)

    // ADT enum support: enum name -> enum_decl* (i8*)
    sv_map adt_enum_decls;

    bool had_error;  // set to true on any compile error; main checks before emitting
...
void ctx_declare_local_volatile(ir_context* ctx, i8* name, i8* alloca_ptr,
                                i8* elem_type, i8* deref_type, bool is_unsigned) {
...
bool ctx_is_local_volatile(ir_context* ctx, i8* name) {
...
i8* ns_name_to_prefix(i8* ns_name) {
    if (ns_name == (i8*)0) { return (i8*)0; }
    i8 tmp[512]; tmp[0] = 0; i32 ti = 0;
    i32 i = 0;
...
void ctx_add_using_ns(ir_context* ctx, i8* ns_name) {
    if (ns_name == (i8*)0) { return; }
    i8* prefix = ns_name_to_prefix(ns_name);
...
i8* ctx_resolve_func(ir_context* ctx, i8* name) {
    i8* v = sv_map_get(&ctx.global_funcs, name);
    if (v != (i8*)0) { return v; }
...
i8* ctx_resolve_var(ir_context* ctx, i8* name) {
    i8* v = sv_map_get(&ctx.global_vars, name);
    if (v != (i8*)0) { return v; }
    if (ctx.using_ns_len == 0) { return (i8*)0; }
    i8 ubuf[512];
    i32 ui = 0;
